...
第二步，把因子df带入回测框架，进行回测。语句为lu=HappyLujie(factor,freq='Q',only_buy=False,group_num=10,buy_num=30,risk_free_rate=0,filename='回测结果')
...
7。参数说明：
①factor：因子矩阵。为一个dataframe格式。因子的格式应为矩阵格式，index为时间戳格式的时间（需为每月月底或每季度最后一天），columns为带.OF后缀的基金代码。
...
class TigerProsperous(object):
    ...
    def deal_with_factor(self,factor,only_buy=False,group_num=10,buy_num=30,risk_free_rate=0,filename='回测结果'):
...
class HappyLujie(object):
    def __init__(self,factor,freq='Q',only_buy=False,group_num=10,buy_num=30,risk_free_rate=0,filename='回测结果'):
        self.lu=TigerProsperous(freq=freq)
        self.lu.read_in_nav()
        self.lu.deal_with_factor(factor=factor,only_buy=only_buy,group_num=group_num,buy_num=buy_num,risk_free_rate=risk_free_rate,filename=filename)
